...
async function init(databaseCredentials) {
...
async function databaseDirectory(params, callback = null) {
  ...
  try {
    ...
    for (var ii = 0; ii < databaseList.length; ii++) {
      ...
      colorPrint(
        "blue",
        `Database: ${ii} of ${databaseList.length} ${databaseList[ii]}`
      );
      try {
        thisDatabase = await createDatabaseConnection(_databaseDetails);
        let { database_type = "" } = _databaseDetails;
        let this_database_SQL =
          database_type == "Snowflake" ? `${this_database_name}.` : "";
        ...
        if (schema === null) {
          ...
          let _schema =
            database_type == "Snowflake" ? schema.toUpperCase() : schema;
        ...
        colorPrint(
          "yellow",
          `# _schemas_and_tables: ${_schemas_and_tables.length}`
        );
